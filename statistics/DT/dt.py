...
def info_gain_train(datasets):
    ...
    for c in range(count):
        ...
        print u'特征({}) - info_gain - {:.3f}'.format(labels[c], c_info_gain)
    # 比较大小
    best_ = max(best_feature, key=lambda x: x[-1])
    return u'特征({})的信息增益最大，选择为根节点特征'.format(labels[best_[0]])
...
class DTree:
    ...
    def train(self, train_data):
        ...
        # print type(y_train),y_train.value_counts().sort_values(ascending=False).index[0];exit(0)
...
# print dt.predict([u'老年', u'否', u'否', u'一般'])
